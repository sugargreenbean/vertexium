...
import static org.elasticsearch.common.geo.parsers.ShapeParser.FIELD_COORDINATES;
import static org.elasticsearch.common.geo.parsers.ShapeParser.FIELD_GEOMETRIES;
import static org.elasticsearch.search.suggest.completion.context.ContextMapping.FIELD_TYPE;
import static org.vertexium.elasticsearch7.ElasticsearchPropertyNameInfo.PROPERTY_NAME_PATTERN;
...
public class Elasticsearch7SearchIndex implements SearchIndex, SearchIndexWithVertexPropertyCountByValue {
    private static final VertexiumLogger LOGGER = VertexiumLoggerFactory.getLogger(Elasticsearch7SearchIndex.class);
    ...
    private Set<String> additionalVisibilitiesCache = ConcurrentHashMap.newKeySet();
    ...
    CompletableFuture<Void> addElement(
        ...
        CompletableFuture<Void> result = bulkUpdateService.addUpdate(
    ...
    private CompletableFuture<Void> addExtendedDataUpdateToBulk(
        ...
        Map<String, String> source = buildSourceForExtendedDataUpsert(graph, elementLocation, tableName, rowId);
        if (MUTATION_LOGGER.isTraceEnabled()) {
        ...
        return bulkUpdateService.addUpdate(
            indexInfo.getIndexName(),
            getIdStrategy().getType(),
            extendedDataDocId,
            elementLocation,
    ...
    public void markElementHidden(Graph graph, Element element, Visibility visibility, Authorizations authorizations) {
        ...
        bulkUpdateService.addUpdate(
            indexName,
            getIdStrategy().getType(),
            getIdStrategy().createElementDocId(element),
            element,
            Collections.emptyMap(),
            Collections.singletonMap(hiddenVisibilityPropertyName, true),
            Collections.emptyList(),
            Collections.emptyMap(),
            Collections.emptyList(),
            Collections.emptyList(),
    ...
    protected void addPropertyNameVisibility(Graph graph, IndexInfo indexInfo, String propertyName, Visibility propertyVisibility) {
        ...
        if (propertyVisibility != null) {
            this.propertyNameVisibilitiesStore.addPropertyNameVisibility(graph, propertyNameNoVisibility, propertyVisibility);
        }
    ...
    protected Map<String, Object> convertGeoPoint(GeoPoint geoPoint) {
        ...
        propertyValueMap.put(FIELD_COORDINATES.getPreferredName(), Arrays.asList(geoPoint.getLongitude(), geoPoint.getLatitude()));
    ...
    protected Map<String, Object> convertGeoCircle(GeoCircle geoCircle) {
        ...
        propertyValueMap.put(FIELD_COORDINATES.getPreferredName(), coordinates);
    ...
    protected Map<String, Object> convertGeoCollection(GeoCollection geoCollection) {
        ...
        propertyValueMap.put(FIELD_GEOMETRIES.getPreferredName(), geometries);
